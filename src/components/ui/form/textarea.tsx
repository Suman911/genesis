'use client'

import { forwardRef, ForwardedRef, useRef, RefObject } from "react";
import { ValidatorFunction } from "./validation";

interface TextAreaProps extends React.HTMLAttributes<HTMLTextAreaElement> {
    placeholder?: string;
    textbox: ForwardedRef<HTMLTextAreaElement>;
    validate: ValidatorFunction;
}

export const TextArea = forwardRef<HTMLTextAreaElement, TextAreaProps>(
    ({ className = "", textbox, validate, ...props }) => {
        const msgbox = useRef<HTMLDivElement>(null);

        return (
            <div className="w-full relative py-4 px-0 sm:py-4">
                <textarea
                    ref={textbox}
                    {...props}
                    name="txtadjbox"
                    className={`h-32 w-full p-2 outline-none border-2 border-primary rounded-xl ${className}`}
                    onBlur={() => validate(textbox as RefObject<HTMLTextAreaElement>, msgbox)}
                    required
                />
                <div ref={msgbox} className="w-full text-sm text-center absolute left-0 top-full -translate-y-3.5 text-red-500 hidden"></div>
            </div>
        );
    }
);

// Fixing the missing display name warning
TextArea.displayName = "TextArea";
